...
import { createTask, completeTask } from "../src/task.ts";
import { TaskStore } from "../src/store.ts";
import { makeServer } from "../src/api.ts";
...
describe("CLI", () => {
  ...
  let store: TaskStore;
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), "node-learn-20-"));
...
describe("HTTP API", () => {
  ...
  let base = "";
  before(async () => {
    dir = await mkdtemp(join(tmpdir(), "node-learn-20h-"));
